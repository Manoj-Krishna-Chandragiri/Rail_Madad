...
class FirebaseAuthMiddleware:
    ...
    def __call__(self, request):
        ...
        if hasattr(settings, 'DEVELOPMENT_MODE') and settings.DEVELOPMENT_MODE and not has_real_token:
            logger.info(f"[DEV] Development mode - Request to: {request.path}")
            logger.info(f"[DEV] Authorization header: {auth_header[:50] if auth_header else 'None'}...")
            ...
            if dev_token_header and dev_token_header.startswith('Bearer dev-face-token-'):
                try:
                    ...
                    logger.info(f"[DEV] Parsing dev face token: {token_body}")
                    parts = token_body.split('-')
                    logger.info(f"[DEV] Token parts: {parts}")
                    if len(parts) >= 4:
                        dev_token_user_id = int(parts[3])
                        logger.info(f"[DEV] Parsed dev face token for user id {dev_token_user_id}")
                except Exception as e:
                    logger.warning(f"[DEV] Failed to parse dev face token: {e}")
