...
const AdminLogin = () => {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Set admin-specific localStorage
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Fetch user profile to validate admin status
      const userProfile = await fetchAndStoreUserProfile();
          ...
          {!showForgotPassword && !showSignUp ? (
            <form className="space-y-6 overflow-y-scroll pr-2" style={{scrollbarWidth: 'thin'}} onSubmit={handleSignIn}>
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 rounded-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              >
